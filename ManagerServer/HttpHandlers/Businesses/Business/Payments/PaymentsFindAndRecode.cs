...
namespace ManagerServer.HttpHandlers.Businesses.Business.Payments
{
    ...
    [Guide("The **Find and Recode** function allows you to search through payment transactions and update multiple line items at once. This feature is particularly useful when you need to correct *account allocations* or *tax codes* across many transactions.")]
    [Guide("You can filter payments by date, bank account, description, account, or tax code using the search box. The search function looks for matching text in any of these fields.")]
    [Header("How to Use Find and Recode")]
    [Guide("To begin, enter search terms in the search box at the top of the screen. Leave the search box empty to view all payment line items. The system displays up to 50 items per page by default.")]
    [Guide("Select the payment line items you want to update by checking the boxes next to each item. You can use the checkbox in the header row to select all visible items on the current page.")]
    [Guide("At the bottom of the screen, choose the new *account* and/or *tax code* from the dropdown menus. Only the fields you select will be updated - leave a dropdown blank to keep the existing values unchanged.")]
    [Guide("Click the **Bulk Update** button to apply your changes to all selected items. The system will update the transactions and refresh the screen.")]
    [Header("Important Notes")]
    [Guide("This function only shows payment line items that are not linked to inventory items or other specific transactions. Line items associated with suppliers, customers, or inventory items must be edited individually.")]
    [Guide("The **Edit** button next to each line item allows you to open the full payment transaction for detailed editing if needed.")]
    internal sealed class PaymentsFindAndRecode : BusinessTemplate
    {
        ...
        protected override void InnerGet2()
        {
            ...
            foreach (var e in ApplicationData.Businesses.Get(Business).OfType<ManagerServer.Model.Payment>())
            {
                if (!e.PaidFrom.HasValue) continue;
                if (!bankAccounts.ContainsKey(e.PaidFrom.Value)) continue;
                ...
                for (int i = 0; i < e.Lines.Length; i++)
                {
                    ...
                    rows.Add(new Row() { Key = e.Key, Index = i, BankAccount = bankAccounts[e.PaidFrom.Value], Account = account, Tax = tax, Amount = e.Lines[i].Amount, Date = e.Date, Description = e.Lines[i].LineDescription.IfEmptyReplaceWith(e.Description) });
                }
            }
...
$('#checkAll').click(function(){ $('input.bulk').not(this).prop('checked', this.checked); });
            ...
            using (Div(@class: "card"))
            {
                ...
                HiddenRows(Term, hidden, new PaymentsFindAndRecode() { BankAccount = BankAccount, Business = Business }.ToUrl());
                if (selection.Length == 0)
                {
                    using (Div(@class: "card-inset text-center p-24"))
                    {
                        using (Div(@class: "card-title")) Write(Strings.Empty);
                    }
                }
                else
                {
                    ...
                    {
                        var index = 0;
                        using (Table(@class: "card-table", style: "margin-bottom: 0px"))
                        {
                            using (THead())
                            {
                                using (Tr())
                            ...
                            foreach (var e in selection)
                            {
                                using (Tr())
                                {
                                    ...
                                    using (Td(style: "width: 1px"))
                                    {
                                        using (A(href: new PaymentForm() { Business = Business, Key = e.Key, Referrer = referrer }.ToUrl(), @class: "btn btn-sm")) Write(Strings.Edit);
                                    }
                        ...
                        using (Div(@class: "card-header"))
                        {
                            using (Table()) using (Tr())
                            {
                                ...
                                using (Td(style: "padding-left: 5px"))
                                {
                                    ...
                                    using (Div()) using (Select(name: "Account", @class: "form-select"))
                                    {
                                        Option();
                                ...
                                if (taxCodes.Any())
                                {
                                    using (Td(style: "padding-left: 5px"))
                                    {
                                        ...
                                        using (Div()) using (Select(name: "TaxCode", @class: "form-select"))
                                        {
                                            Option();
        ...
        private void Pagination(int total)
        {
            ...
            var handler = (PaymentsFindAndRecode)this.MemberwiseClone();
        ...
        protected override async Task InnerPost()
        {
            ...
            var bankTransactions = ApplicationData.Businesses.Get(Business).OfType<ManagerServer.Model.Payment>().ToDictionary(x => x.Key);

            var list = new List<ManagerServer.Model.Payment>();
